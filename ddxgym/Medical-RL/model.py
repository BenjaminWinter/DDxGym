...
class IMPALATransformer(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name, model_name_or_path="bert-base-uncased", num_diseases=256, loss_scale=1.0):
        ...
        # classifier_dropout = (
        #     self.encoder_config.classifier_dropout if self.encoder_config.classifier_dropout is not None else self.encoder_config.hidden_dropout_prob
        # )
...
class IMPALATransformerDiseasePrediction(IMPALATransformer):
    def __init__(self, obs_space, action_space, num_outputs, model_config, name, model_name_or_path="bert-base-uncased", num_diseases=256, loss_scale=0.66):
    ...
    def custom_loss(self, policy_loss, loss_inputs):
        ...
        return [ploss + self.loss_scale * supervised_loss + self.loss_scale * masked_lm_loss for ploss in policy_loss]
...
class IMPALATransformerLMDiseasePrediction(IMPALATransformerLM):
    def __init__(self, obs_space, action_space, num_outputs, model_config, name, model_name_or_path="bert-base-uncased", num_diseases=256, loss_scale=1.0):
        IMPALATransformerLM.__init__(self, obs_space, action_space, num_outputs, model_config, name, model_name_or_path="bert-base-uncased", num_diseases=256, loss_scale=1.0)
...
#class IMPALAFruitfly(TorchRNN, nn.Module):
    ...
    def forward(self, input_dict: Dict, state: List, seq_lens: Any):
        ...
        #logits = self.activation(self.action_outputs(activations*binary_hash))
        #logits = self.activation(self.action_outputs(activations))
...
class IMPALAFullyConnected(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name, model_name_or_path="bert-base-uncased", num_diseases=256, loss_scale=1.0):
        ...
        # self.action_outputs = nn.Linear(768, num_outputs)
    ...
    def forward(self, input_dict: Dict, state: List, seq_lens: Any):
        ...
        inp = input_dict["obs"].squeeze(1).type(torch.FloatTensor).cuda()
        #print(f"data device: {inp.device}")
        #print(f"model device: {self.encoder.device}")
        
        encoded = self.encoder(inp)


        # self._features = encoded

        # action_logits = self.action_outputs(encoded)
        #print(f"encoded size: {encoded.size()}")
        return encoded, []
        
    def value_function(self):
        print(f"value shape: {(self._last_batch_size,)}")
        return torch.from_numpy(np.zeros(shape=(self._last_batch_size,)))
    
class IMPALALSTM(TorchModelV2, nn.Module):
    ...
    def __init__(self, obs_space, action_space, num_outputs, model_config, name, model_name_or_path="bert-base-uncased", num_diseases=256, loss_scale=1.0):
        ...
        self.config = model_config
        self.encoder_config = AutoConfig.from_pretrained(model_name_or_path)
        self.embeddings = AutoModel.from_pretrained(model_name_or_path).embeddings.word_embeddings

        self.encoder = nn.LSTM(self.encoder_config.hidden_size, self.encoder_config.hidden_size, batch_first=True, dropout=0.1, num_layers=3)
    ...
    def forward(self, input_dict: Dict, state: List, seq_lens: Any):
        ...
        encoded, hidden = self.encoder(self.embeddings(input_dict["obs"].squeeze(1)))

        self._features = torch.mean(self.dropout(encoded), dim=1)

        action_logits = self.action_outputs(self._features)
