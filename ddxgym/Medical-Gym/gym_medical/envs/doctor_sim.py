...
from ..preprocessing import process_csv, Disease, Procedure
            ...
            if random.random() < s["probability"] and not s["symptom"].is_main:
                if s["firstDay"] == 0:
                    ...
                    if s["firstDay"] not in self.future_symptoms:
                        ...
                        self.future_symptoms[s["firstDay"]].append(s["symptom"])
        if self.symptoms == []:
            self.symptoms.append(random.choice([s for s in self.disease.symptoms.values() if not s["symptom"].is_main])["symptom"])
        ...
        if procedure.type == PROCEDURE_TYPE.index("Examination"):
            ...
            for symptom in undiscovered:
                if procedure in symptom.examinations:
                    ...
                    if procedure.id == "EXM_INTERVIEW": # nerf interview action to only uncover one symptom at a time
                        break
            self.discovered_symptoms += new
        #check if we treat a revealed symptom
        else:
            for symptom in (set(self.discovered_symptoms) - set(self.treated_symptoms)):
...
class TextSpace(Space):
    ...
    def contains(self, x):
        if isinstance(x, list) or isinstance(x, np.ndarray):
            ...
            print("TextSpace expected a list or numpy array but got " + str(type(x)))
            return False
        return True

    def __repr__(self):
        return f"TextSpace: obs length: {self.length}, Vocab: {self.vocab_size}"

    def __eq__(self, other):
        return isinstance(other, TextSpace) and self.length == other.length and self.vocab_size == other.length
...
class DoctorSim(gym.Env):
    ...
    def __init__(self, max_episode_steps=100, observation_length=512, tokenizer="bert-base-uncased", data_path="../../data/project_hospital", max_diseases = None, fruitfly = False, is_csv = True):
        ...
        if tokenizer == "lstm":
            self.tokenizer = AutoTokenizer.from_pretrained("prajjwal1/bert-small", use_fast=True)
        else:
            self.tokenizer = AutoTokenizer.from_pretrained(tokenizer, use_fast=True)
        self.fruitfly = fruitfly # Change the way the tokenization happens in the case of the Fruitfly model
        
        if is_csv:
            self.diseases, self.symptoms, self.procedures = process_csv(data_path, max_diseases = max_diseases)
            self.procedures_to_actions = open(os.path.join(data_path, "actions_csv.txt")).read().splitlines()
            
        self.procedures_to_actions = [a for a in self.procedures_to_actions if a in self.procedures.keys()]
        ...
        self.observation_space  = TextSpace(observation_length, len(self.tokenizer))
    ...
    def step(self, action: int):
        ...
        obs, r, done = self._step(self.procedures[self.procedures_to_actions[action]])
        

        return self.tokenize(obs), r, done, {"disease_target": self._disease_target}
    ...
    def reset_with_disease(self, disease_name: str):
        self._disease_target = [x for x in self.diseases.values() if x.name == disease_name][0]
        self._patient = Patient(self._disease_target)
        self.procedure_reward = 0
